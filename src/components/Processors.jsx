...
const Processors = () => {
    ...
    useEffect(() => {
        const getGoods = async () => {
            setLoading(true);
            const response = await fetch("http://127.0.0.1:8000/processors");
            if (componentMounted) {
    ...
    const Search = () => {
        ...
        let processorSearch = data.filter(item => {
        ...
        return(
            <section className="py-4 container">
                <div className="row justify-content-center">
                    ...
                    {processorSearch.map((item, index) =>{
                        item.quantity = 1;
                        return(
                            <>
                                <div className="col-11 col-md-6 col-lg-3 mx-0 mb-4" key ={index}>
                                    <div className="card p-0 overflow-hidden h-100 shadow">
                                        ...
                                        <div className="card-body">
                                            ...
                                            <h7 className="card-text">{item.desc}</h7>
                                            <h5 className="card-text">{item.price} руб.</h5>
                                            <h5 className="card-text">key = {item.id}</h5>
